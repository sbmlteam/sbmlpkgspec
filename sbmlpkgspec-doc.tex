...
\packageVersion{Version 1.3.0}
\packageVersionDate{24 July 2012}
...
installation of \LaTeXe.  I developed and tested this document
class with the TeX Live 2011 distribution on a Mac~OS~X 10.6.x system, and it
is has been reported to work with TeX Live on Windows and Ubuntu Linux.  (For
Ubuntu, make sure to install the following packages: ``\texttt{texlive}'',
...
\item \emph{Per-document installation}.  This is arguably the simplest
  ...
  document class as explained in \sec{usage}), it will find the
...
A final point about \sbmlpkg is worth mentioning right at the
...
typically have to run the command not twice, but \notice \emph{three} times, to get
...
As shown in \tab{ref-commands}, the \sbmlpkg class also defines starred
versions of the commands.  These are useful when the item being referenced
is located on another page, and you want to refer to it more than once from
the text of a given paragraph.  \notice  The regular version of the commands 
such as ``\texttt{\textbackslash fig\{}\emph{label\,}\texttt{\}}'' will always
produce a page reference (e.g., ``see Figure~2.5 on the following page''),
which becomes rather tedious to read if there is more than one occurrence of
it in the same paragraph.  To avoid that, use the normal version of the
...
red (or more precisely, the color named \val{sbmlchangedcolor}, which
is defined by \sbmlpkg as a maroon red), \changed{like this}.  It can be used
for long stretches of text and include embedded spaces and formatting
...
\item[\color{black}\textbackslash defRef\{\emph{name}\}\{\emph{section}\}] Create a
...
\item[\color{black}\textbackslash absDefRef\{\emph{name}\}\{\emph{section}\}] Create a
  hyperlinked reference to the section labeled \emph{section} and call it
  \emph{name}.  The reference is inserted at the point in the text where
  the \texttt{\textbackslash defRef} command is invoked.  This command is
...
\item[draftspec] This option causes the front page of the document
