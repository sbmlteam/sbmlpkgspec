...
\packageVersion{Version 1.4.0}
\packageVersionDate{24 July 2013}
...
versions of the commands.  These are useful when the item being referenced is
located on another page, and you want to refer to it more than once from the
text of a given paragraph.  \notice The regular version of the commands such
as ``\texttt{\textbackslash fig\{}\emph{label\,}\texttt{\}}'' will always
...
should likewise define their own validation and consistency rules.  The SBML
convention identifies different degrees of strictness.  The differences are
expressed in the statement of a rule: either a rule states a condition
\emph{must} be true, or a rule states that it \emph{should} be true.  Rules
of the former kind are SBML validation rules---a model encoded in SBML must
conform to all of them in order to be considered valid.  Rules of the latter
kind are consistency rules.
